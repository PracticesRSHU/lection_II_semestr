...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        mainbar.add_cascade(label="Таблиця Computers ", command=self.addNotebookComputers)
    ...
    def addNotebookUsers(self):
        ...
        print(self.myNotebook.index(self.frameUsers))
        # self.myNotebook.add(self.frameUsers, text="Users")
    ...
    def refreshCurrentTabNotebook(self,event):
        if len(self.myNotebook.tabs())!=0:
            # визначення номера активної вкладки
            currentTab = self.myNotebook.tabs().index(self.myNotebook.select())
            # в залежності від вибрано вкладки виконувати виклик діалогового вікна
    ...
    def open_add_dialog(self):
        # визначення номера активної вкладки
        currentTab = self.myNotebook.tabs().index(self.myNotebook.select())
        # в залежності від вибраної вкладки виконувати виклик діалогового вікна
        if self.myNotebook.tab(currentTab)["text"] == "Computers":
    ...
    def view_records_computers(self):
        data = self.db.selectAllComputerts()
        # self.table_computers()
    ...
    def table_users(self):
        ...
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.treeUsers.configure(yscrollcommand=scrollbar.set)
...
class WindowInputDialogOrder(tk.Toplevel):
    ...
    def init_input(self):
        ...
        print(self.db.selectAllUsers())
        self.entry_user_id = ttk.Combobox(self,value=list(self.db.selectAllUsers()))
        ...
        def add_date(event):
            self.view.records_orders(self.entry_user_id.get()[0],
...
class WindowEditDialogOrder(WindowInputDialogOrder):
    ...
    def init_edit(self):
        ...
        btn_edit = ttk.Button(self, text="Редагувати")
        btn_edit.place(x=250, y=140)
        btn_edit.bind("<Button-1>", lambda event: self.view.update_order(self.entry_user_id.get()[0],
                                                                         self.entry_computers_id.get(),
    ...
    def default_value(self):
        ...
        result = self.db.curs.fetchone()
        self.entry_user_id.insert(0,self.db.selectUserById(result[1]))
...
class DB():
    ...
    # CRUD operations with table 'computers'
    def selectAllComputerts(self):
        self.curs.execute("""SELECT * FROM computers""")
        return self.curs.fetchall()

    def selectComputertById(self):
        self.curs.execute("""SELECT * FROM computers WHERE id=?""", (id,))
        return self.curs.fetchall()

    def input_data_computers(self, name, count, price):
        self.curs.execute('INSERT INTO computers (name, count, price) VALUES(?,?,?)', (name, count, price))
        self.conn.commit()
    ...
    def selectUserById(self, id):
        self.curs.execute("""SELECT * FROM users WHERE id=?""", (id,))
